...
### 2.4 关于 LSP（lsp-config.json）

Snow App 目前**不运行完整的 LSP**：`codelens` 是内置静态分析，不启动
外部语言服务器进程，开箱即用、无需安装任何语言服务器。

`~/.snow/lsp-config.json`（scope: `lsp-config`）是**预留的语言服务器配置域**：
`config` 工具可读写并校验该文件（字段结构与示例见
[3-配置文件字段参考](../3-参考手册/3-配置文件字段参考.md) 第 10 节），
但当前版本尚未接入 LSP 客户端运行时，写入后**不会改变任何行为**。
