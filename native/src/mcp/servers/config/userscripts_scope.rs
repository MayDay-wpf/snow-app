...
pub fn list_userscripts(db_path: &Path) -> napi::Result<Value> {
    ...
    Ok(json!({
        ...
        "items": items,
        "count": items.len(),
        "guidance": "USERSCRIPTS (Tampermonkey-compatible) - install a userscript written for the user.\nRECOMMENDED INSTALL FLOW (avoids huge tool args): 1) write the full source to a file with filesystem-create (or edit it with filesystem-replace_edit), e.g. ./scripts/demo.user.js; 2) config-set scope=userscripts key=\"new\" value={sourcePath: \"/abs/path/to/demo.user.js\"} - the backend reads the file, parses // ==UserScript== metadata, writes the DB row and copies the file to ~/.snowapp/browser-script/{script_id}.user.js. UPDATE: edit the same file, then config-set scope=userscripts key=<existing scriptId> value={sourcePath: ...}. Small scripts may also be inlined with value={raw: \"...\"}.\nTOGGLE: config-set scope=userscripts key=<scriptId> value={enabled: true|false}\nGM VALUES (GM_getValue/GM_setValue persistence): config-set scope=userscripts key=<scriptId> value={values: {\"k\":\"v\"}}; remove one with value={deleteValues:[\"k\"]}\nREAD: config-get scope=userscripts key=<scriptId> (returns metadata + full source + GM values)\nLIST: config-list scope=userscripts\nUNINSTALL: config-delete scope=userscripts key=<scriptId> confirmed=true (deletes DB row + file)\nCLIENT UI SCRIPTS (customize the Snow desktop window itself instead of the built-in browser): add `@snow-target client` in the metadata header (or write `@match snow://client/<view>`), then the script is injected into the desktop window. Extra directives: `@snow-target client|browser|all` (default browser), `@snow-view chat,plugins` (restrict to main views, `*` = any view), `@snow-surface chat,sidebar` (restrict to layout surfaces: chat / sidebar / topbar / right-panel / settings), `@snow-scope global` (run once at app start and stay resident across view switches), `@snow-sandbox false` (run in the main world with full app permissions - needs explicit user confirmation; `@grant unsafeWindow` implies it). Client scripts run sandboxed by default (no access to window.snow) and get GM_* plus the `snow` API. DOM: an isolated-world script shares the page DOM, so browser-native DOM APIs work directly (document.querySelectorAll, createElement/append/remove, MutationObserver, addEventListener, element.style) - customize ANY part of the window the way a Chrome userscript does; call element.click() or dispatchEvent() to drive the UI, and use a MutationObserver when React may re-create the nodes. Stable hooks to select: read-only anchors via data-snow-anchor - app.root, topbar, sidebar, sidebar.nav, sidebar.footer, main.view (with data-snow-view), chat.messages, chat.message (per message; plus data-snow-message-id / data-snow-message-role), chat.input, rightPanel, rightPanel.tabs, rightPanel.content; writable slots via data-snow-slot (append nodes into them; the host empties used slots on unload) - topbar.actions, sidebar.nav.actions, sidebar.footer.actions, chat.input.actions, chat.message.actions (per message). The `snow` API only covers what DOM cannot: snow.onCleanup(fn) / snow.style(css) / snow.anchor(name) / snow.slot(name); events snow.on('context'|'view-enter'|'view-leave'|'conversation-change'|'stream-start'|'stream-end'|'theme-change') with snow.context carrying view/surfaces/tabs/theme/locale/projectId/conversationId/isStreaming/appReady; app actions snow.client.insertInputText(text) / snow.client.sendMessage(text) / snow.client.openView(viewId) / snow.client.openSettings(viewId) - these are dispatched by the host in the main world because an isolated world cannot deliver CustomEvent detail reliably.
